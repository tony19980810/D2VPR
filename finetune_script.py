...
class my_small_Model(L.LightningModule):
    def __init__(
            self,
            lr=1e-4,
            lr_mul=0.1,
        ...
        super().__init__()
        self.backbone = D2VPR.D2VPR(cross_image_encoder=False)

     
        self.backbone.load_state_dict(
                torch.load('./your_distillation_weights.pth')['model_state_dict'])
        ...
        for name, param in self.backbone.named_parameters():
            ...
            if 'blocks.9' in name or 'blocks.10' in name or 'blocks.11' in name or 'lin' in name or 'up_norm' in name or "aggregation" in name or 'blocks.8' in name or 'encoder' in name or 'topdown' in name or 'offset' in name:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # teacher_descriptors,_=self.teacher(images)
        # compute loss
