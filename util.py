...
def resume_model(args, model):
    ...
    # state_dict = OrderedDict({k: v for k, v in state_dict.items() if 'adapter' in k or 'encoder' in k})
...
def compute_pca(args, model, pca_dataset_folder, full_features_dim):
    ...
    max_samples = min(len(pca_ds), 2 ** 14)
    pca_features = np.empty([max_samples, full_features_dim])

    filled = 0  

    with torch.no_grad():
        for images in dl:
            features = model(images.cuda()).cpu().numpy()

          
            space_left = max_samples - filled
            if space_left <= 0:
                break

        
            take = min(space_left, len(features))

            pca_features[filled: filled + take] = features[:take]
            filled += take

  
    pca = PCA(args.pca_dim)
    pca.fit(pca_features[:filled])
    return pca
